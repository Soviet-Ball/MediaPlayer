...
QString getlastPath_private(QString path)
{
    QString ret = path;
    ...
    {
        if(QCoreApplication::applicationFilePath().endsWith(".exe") || QCoreApplication::applicationFilePath().endsWith(".EXE"))//Windows Only
        {
            ret = "我的电脑";
        }
    }
    ...
    m_fileSystemModel(new MyFileSystemModel(this))
{
    ...
    QStringList list;
#ifdef Q_OS_WIN
    list << "选择常用路径" << "我的电脑" << Windows::getDrivers() << "桌面" << "下载" << "文档" << "当前路径";
#else
#ifdef Q_OS_ANDROID
